  ...
  describe('ExtUtil Module', () => {
    describe('Alphabet Reading', () => {
      ...
      it('can read/write alphabet', () => {
        _.range(50).forEach(() => {
          const sample = _.sampleSize(examples, 6);
          const original = sample.map((t) => t[0]);
          const korean = sample.map((t) => t[1]);
          const originalStr = original.join('');
          const koreanStr = korean.join('');
    ...
    describe('Hanja Reading', () => {
      ...
      it('can recognize Hanja', () => {
        ...
        expect(ExtUtil.isHanja("⺀⺁⺂⻱⻲⻳").every((x: any) => x)).toBe(true)
      });
    });
    ...
    describe('Hangul Processor', () => {
      it('identifies hangul character', () => {
        ...
        for (const [ch, rec] of _.zip(hangulSentence.split(''), recognized)) {
          expect(ch !== ' ').toBe(rec);
        }
        ...
        for (const [ch, a, h, ih, j1, j2, j3] of (_.zip as any)(mixedSentence.split(''), isCompleteHangul, isHangul, isIncompleteHangul, isChosungJamo, isJungsungJamo, isJongsungJamo)) {
      ...
      it('dissembles hangul', () => {
        ...
        for (const [a, j1, j2, j3] of _.zip(completeHangul, chosung, jungsung, jongsung)) {
          if (a) {
            expect(ExtUtil.isChosungJamo(j1!).every((x: any) => x)).toBe(true);
            expect(ExtUtil.isJungsungJamo(j1!).some((x: any) => x)).toBe(false);
            expect(ExtUtil.isJongsungJamo(j1!).some((x: any) => x)).toBe(false);
            ...
            expect(ExtUtil.isChosungJamo(j2!).some((x: any) => x)).toBe(false);
            expect(ExtUtil.isJungsungJamo(j2!).every((x: any) => x)).toBe(true);
            expect(ExtUtil.isJongsungJamo(j2!).some((x: any) => x)).toBe(false);
            ...
            if (typeof j3 !== 'undefined') {
              expect(ExtUtil.isChosungJamo(j3).some((x: any) => x)).toBe(false);
              expect(ExtUtil.isJungsungJamo(j3).some((x: any) => x)).toBe(false);
              expect(ExtUtil.isJongsungJamo(j3).every((x: any) => x)).toBe(true);
      ...
      it('assembles hangul', () => {
        _.range(100).forEach(() => {
          ...
          let codestr = code.map((c) => c.filter((x: any) => typeof x !== 'undefined').join('')).join('');
          ...
          expect(ExtUtil.getChosung(str)).toEqual(code.map((x: any) => x[0]));
          expect(ExtUtil.getJungsung(str)).toEqual(code.map((x: any) => x[1]));
          expect(ExtUtil.getJongsung(str)).toEqual(code.map((x: any) => x[2]));
